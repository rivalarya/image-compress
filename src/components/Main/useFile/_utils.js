...
const effect = {
    ...
    dragEnter: () => {
        ...
        document.querySelector('.input-file').style.boxShadow = '0 0 4px 1px black';
    },
    onDragLeave: () => {
        document.querySelector('.input-file').style.opacity = 1;
        document.querySelector('.input-file').style.boxShadow = '0 0 2px black;';
    },
    finish: () => {
        setTimeout(() => {          
        document.querySelector('input[type=file]').removeAttribute('disabled');
        document.querySelector('.choice-file').innerHTML = `<span class="lds-dual-ring" style="display: none; margin:auto;"></span>
                    <p> Click on this area to choose <span> or drag and drop </span>your image <span>here</span></p>`; // diambil dari main.js
        }, 1000);
    }
}
